...
#------------------------------------------------------------------------------------------------------------
 ...
 BackupTmpDir=$HOME/$(date +%F-%H%M)_"$USER"@"$HOSTNAME"_backup
 BackupTmpSubDir=$( awk -F'/' '{print $4}' <<< $BackupTmpDir)
 BackupArchiv=$HOME/$(date +%F-%H%M)_"$USER"@"$HOSTNAME"_backup.tar.gz
...
# 2.2	=> change compressor from 7z to tar.gz to include symlinks
# 2.1	=> change chown syntax $user:$user
# 2.0	=> write changes
