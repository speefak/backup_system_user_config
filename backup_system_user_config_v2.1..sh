...
#------------------------------------------------------------------------------------------------------------
 ...
 BackupTmpDir=$HOME/$(date +%F-%H%M)_"$USER"@"$HOSTNAME"_backup		#TODO if root executio user tmp dir /home/ instead /root to avoid diskspacefull error
 BackupArchiv=$HOME/$(date +%F-%H%M)_"$USER"@"$HOSTNAME"_backup.7z
